package domain;
import domain.player.PlayerClass;
...
public class BattleScene {
    ...
    private Boolean makeHit(FantasyCharacter defender, FantasyCharacter attacker, Realm.FightCallback fightCallback) {
        ...
        if (hit != 0) {
            System.out.println(String.format("%s Нанес удар в %d едениц!", attacker.getName(), hit));
            System.out.println(String.format("У %s осталось %d едениц здоровья...", defender.getName(), defenderHealth));
        } else {
            System.out.println(String.format("%s промахнулся!", attacker.getName()));
        }
        if (defenderHealth <= 0 && defender instanceof PlayerClass) {
            System.out.println("Извните вы пали в бою...");
            fightCallback.fightLost();
            return true;
        } else if(defenderHealth <= 0) {
            System.out.println(String.format("Враг повержен! Вы порлучаете %d опты и %d золота", defender.getXp(), defender.getGold()));
